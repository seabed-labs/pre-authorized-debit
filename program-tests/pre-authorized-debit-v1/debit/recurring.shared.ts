    ...
    describe(`pre-authorized-debit-v1#debit (recurring) ${testSuffix}`, () => {
        ...
        async function setupRecurringPreAuthorization(
            ...
            await program.methods
                .initPreAuthorization({
                    variant: {
                        recurring: {
                            repeatFrequencySeconds: new anchor.BN(
                            ...
                            numCycles: numCycles
                                ? new anchor.BN(numCycles)
                                : null,
                            resetEveryCycle,
                        },
                    },
                    debitAuthority: debitAuthorityKeypair.publicKey,
                    activationUnixTimestamp: new anchor.BN(
        ...
        context("non-cumulative, unlimited recurring pre-authorization", () => {
            ...
            it("allows debit_authority to debit funds every cycle (cycled 2x)", async () => {
                ...
                expect(
                    destinationTokenAccountBefore.amount.toString(),
                ).to.equal("0");
                ...
                expect(
                    sourceTokenAccountBefore.delegatedAmount.toString(),
                ).to.equal(U64_MAX);
                expect(
                ...
                for (let i = 1; i <= 2; i++) {
                    ...
                    expect(
                        destinationTokenAccountAfter.amount.toString(),
                    ).to.equal((33e6 * i).toString());
                    expect(sourceTokenAccountAfter.amount.toString()).to.equal(
                        (1000e6 - 33e6 * i).toString(),
                    );
                    expect(
                        sourceTokenAccountAfter.delegate?.toBase58(),
                    ).to.equal(smartDelegatePubkey.toBase58());
                    expect(
                        sourceTokenAccountAfter.delegatedAmount.toString(),
                    ).to.equal((BigInt(U64_MAX) - BigInt(33e6 * i)).toString());
            ...
            it("allows partial debits within a cycle", async () => {
                ...
                for (let i = 1; i <= 2; i++) {
                    ...
                    expect(
                        sourceTokenAccountAfter.delegatedAmount.toString(),
                    ).to.equal((BigInt(U64_MAX) - BigInt(10e6 * i)).toString());
        ...
        context(
            ...
            () => {
                beforeEach(async () => {
                    ...
                    preAuthorizationPubkey =
                        await setupRecurringPreAuthorization(
                            activationUnixTimestamp,
                            3, // 3 second repeat frequency
                            BigInt(33e6), // authorize 33 tokens each cycle
        ...
        context("cumulative, unlimited recurring pre-authorization", () => {
            ...
            it("allows debiting accumulated available amount", async () => {
                ...
                expect(
                    sourceTokenAccountAfter.delegatedAmount.toString(),
                ).to.equal((BigInt(U64_MAX) - BigInt(66e6)).toString());
        ...
        context("generic checks that are similar to one-time debits", () => {
            ...
            it("fires the DebitEvent event", async () => {
                ...
                expect(
                    debitEvent.data.sourceTokenAccountOwner.toString(),
                ).to.equal(userKeypair.publicKey.toBase58());
                expect(
                ...
                expect(
                    debitEvent.data.destinationTokenAccount.toString(),
                ).to.equal(destinationTokenAccountPubkey.toBase58());
                expect(
